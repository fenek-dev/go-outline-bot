...
// Limit represents data limit in bytes
type Limit struct {
...
// Endpoint https://myserver/SecretPath/access-keys/{id}/data-limit
// Response "{\"limit\": {\"bytes\": 10000}}"
func (vpn *OutlineVPN) SetKeyLimit(id string, limit uint64) error {
	request := fasthttp.AcquireRequest()
	response := fasthttp.AcquireResponse()
	defer fasthttp.ReleaseRequest(request)
	defer fasthttp.ReleaseResponse(response)

	requestBody := LimitRequestBody{
		Limit: Limit{
			Bytes: limit,
		},
	}
	...
	// Set request URI to apiURL/access-keys/id/data-limit
	request.SetRequestURI(fmt.Sprintf("%s/access-keys/%s/data-limit", vpn.apiURL, id))
	...
	// If key is limited, status code always must be 204
	if response.StatusCode() != fasthttp.StatusNoContent {
		return fmt.Errorf("error setting key limit: %v", string(response.Body()))
	}

	fmt.Printf("Key %s limit set to %d bytes: %v\n", id, limit, response.StatusCode())

	return nil
}

func (vpn *OutlineVPN) RemoveKeyLimit(id string) error {
	request := fasthttp.AcquireRequest()
	response := fasthttp.AcquireResponse()
	defer fasthttp.ReleaseRequest(request)
	defer fasthttp.ReleaseResponse(response)
	...
	// If key is limited, status code always must be 204
	if response.StatusCode() != fasthttp.StatusNoContent {
		return fmt.Errorf("error resetting key limit: %v", string(response.Body()))
	}

	fmt.Printf("Key %s limit reset: %v\n", id, response.StatusCode())
